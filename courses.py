...
def main():
    ...
    out.close()
    print("To see the results open file: " + filename)
...
def process_network_courses():
    ...
    for course in network_elect:
        asterisk_course = "*" + course #inserts an asterisk in front of ELECTIVE course name
...
def display_network_courses():
    global out
    out.write(dash_line)
    out.write("\nAAS Degree: Information Technology\n")
    out.write(dash_line)
...
def process_info_courses():
    global info_elect #this must be global since it is changed in this function
    global num_info_req, num_info_elect, tot_info, all_info_courses, out
    temp_set = set() #create a temporary empty set
...
def display_info_courses():
    global out
    out.write("\nCERTIFICATE: Computer & info Support Technologies \n")
    out.write(dash_line)
